...
| **Staff & roles** | Owner, Admin, Registrar, Accountant, Teacher, Viewer. Registrars manage students and enrolments; accountants manage fees and payments; admins and the owner manage settings and staff (including removing someone's access); viewers read only. |
| **Security** | Security headers (HSTS, frame-deny, nosniff, restrictive permissions-policy). Login lockout after 5 failed attempts (15 minutes, DB-backed). Duplicate national IDs rejected on admission and on edit. |
